...
public:
	...
	BOOL Accept(const char* szIP, unsigned short nPort) {
		...
		//3.开启线程
		InitializeCriticalSection(&m_CriSend); //初始化一个临界区对象:临界区锁？
	...
	static DWORD WINAPI RecvThread(LPVOID param) {
		...
		while (1) {
			...
			if (pPkgRecv->m_chFlag & PT_ACK) {  //回答
				EnterCriticalSection(&pMyTCP->m_CriSend);
			...
			else if (pPkgRecv->m_chFlag & PT_PSH) {//数据
			   //校验
			...
			else if (pPkgRecv->m_chFlag & PT_FIN) {//断开
				//发送确认
